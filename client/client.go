...
func (f *crdclient) Create(obj *crd.Image) (*crd.Image, error) {
	var result crd.Image
	err := f.cl.Post().
...
func (f *crdclient) Update(obj *crd.Image) (*crd.Image, error) {
	var result crd.Image
	err := f.cl.Put().
		Namespace(f.ns).Resource(f.plural).
		Body(obj).Do().Into(&result)
...
func (f *crdclient) Get(name string) (*crd.Image, error) {
	var result crd.Image
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
...
func (f *crdclient) List(opts meta_v1.ListOptions) (*crd.ImageList, error) {
	var result crd.ImageList
	err := f.cl.Get().
		Namespace(f.ns).Resource(f.plural).
